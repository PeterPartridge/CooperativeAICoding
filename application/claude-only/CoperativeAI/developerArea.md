...
## Round 83 — the return leg: what the agent said, and where its changes were

### My Feedback

> The AI has finished running, but I cannot see a new branch, changes or AI
> feedback. [Claude's own report pasted: attempt-4, NameGreeter.cs,
> ConsoleApp.cs, tests, dotnet build 0 warnings, dotnet test 15 passed.]
>
> Did the AI when finished give feedback and technical debt and log it as we
> expect in the project. Did the AI prompt generated by claude using all this
> information fit with what the goals of the project are trying to achieve?

### What was actually true

**The branch and the changes were both there.** `hello/hello-world` has the
worktree `.coperativeai-worktrees/askforname` on branch `AskForName`, with the
agent's files uncommitted in it — which is exactly what the brief asked for.
Three separate things made it invisible:

1. **The app reviewed the wrong folder.** `review_solution_changes` read the
   Solution's `local_path` — the clean main checkout — while the work sat in the
   run's own worktree next door. It answered "nothing has changed", truthfully,
   about a folder nobody had changed. It now takes the run and reads that
   checkout; without a run it reads the workspace, which is what it always did.
2. **Nothing ever asked for feedback.** The brief's closing section said only
   "leave the change uncommitted". So the agent wrote a careful account of what
   it built, what it proved and what it left behind — into its terminal, which
   closes with it. A panel called "AI feedback" sat empty while the feedback
   existed on the other side of the glass. That was the real gap: the framework
   is a round trip and only the outward leg was built.
3. **Nothing read one back**, which is the same gap from the other end.

### The prompt itself, read against what it is for

Answering the second question honestly: it mostly fits — the job, the answers
already given, the rules, the chosen approach, the schemas and what waits on
this work all reach the agent in one go, which is the point. Two things did not:

- **Every developer rule appeared twice.** Not a bug in the writer, which emits
  each rule once. The rule *fields* contain the text twice, because inserting a
  template appends — deliberately, so nobody loses a paragraph they wrote for
  looking at a template — and the same template was applied twice. Working as
  designed and still wrong on the page; left as data to fix in Develop → Rules
  rather than changed under the user, and noted as debt: nothing warns you that
  a template's text is already in the box.
- **A console app was told to check its change in the browser**, and two lines
  later that it does not reload on its own. The browser sentence was printed for
  anything with a start command. Whether detection found a watcher is the
  evidence — a self-reloading dev server is the front end — so that now decides
  the wording, and everything else is told to run it and watch it work.

### Implemented

- `review_solution_changes` takes the run and reads its worktree.
- The brief asks for a round record at `.coperativeai/feedback/<item>.md`, under
  fixed headings, and says why: the terminal closes with you, that file is read
  back and shown to the people who asked for the work.
- `agent::record` parses one back — lenient about order, casing and heading
  depth, strict about which section is which, keeping words written under the
  agent's own headings rather than dropping them.
- `read_agent_record` reads it out of the run's checkout; missing is "nothing
  back yet", not an error.
- The AI feedback panel shows it, sections the agent left blank left out.
- The attempt number is now the run being prepared rather than the one before
  it, so the first two attempts stop sharing a brief file.

### Tests

cargo 728/728 (23 ignored), Vitest 708/708, `tsc --noEmit`, clippy
`-D warnings` and `npm run build` clean.

### Your Feedback

- **The record is shown, not yet acted on.** Technical debt read back is a
  paragraph on a panel; it does not become a work item, and "what I could not
  do" does not become the `cantImplement` rows the panel already knows how to
  answer. That join is the obvious next round.
- **Nothing makes the agent write it.** The brief asks; an agent that ignores
  the ask leaves no record, and the app says "not written yet" forever without
  distinguishing that from "still working".

### Technical Debt

- The round record is displayed but not filed: no debt-to-work-item, no
  could-not-do into the answerable list.
- No warning when a rule template's text is already in the field it would be
  appended to (this round's duplicated rules).
