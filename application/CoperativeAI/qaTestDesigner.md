...
- Round 3 (my feedback): It's still the tripod — build the QA-to-AI test generation. *(Development is where I know most and where the app has grown; the other areas need input. The strict rule was confirmed: a scenario must be associated with a **work item** to be implemented by AI, because the deny-by-default policy belongs to a work item — a Deliverable-linked case borrowing the coarser Product policy would let AI write tests for work nobody has planned yet.)*
- Round 4 (my feedback): Check the code is single-use, extendable and not duplicating code — do the rule of three. *(And, asked mid-build: "are we making components reusable?" — which turned the frontend fix from a string helper into a shared `BlockedNote` / `Notice` component used by all eight sites.)*
- Round 5 (my feedback): Continue. *(Finished what round 4 left as debt — `ai_run::Call` now covers the `ok` and `declined` branches too, not just the failure one.)*
- Round 6 (my feedback): Run the generated test — and fold in the test names, cargo is what I use. *(Names give two things: narrowing the run, and attributing a verdict inside a whole-suite run. The second is what makes cargo trustworthy, since cargo cannot be narrowed by file path.)*
