...
- Round 2 (record correction, 2026-09-15): **Still `filled`, and deliberately so.** Round 1 built only the decision half. `src-tauri/src/mcp/mod.rs` is one line (`pub mod decide;`); `decide.rs` holds seven functions marked `#[allow(dead_code)] // Reached by the listener in the next round`; five Tauri commands are registered in `main.rs` with no frontend caller and no Admin panel; and `commands/mcp.rs` says in its own module doc that `enabled` records an intention rather than a running socket. The listener, the panel and the `rmcp` wiring are the remaining work. This bullet exists because the status was audited and found correct — the brief is not stale, the feature is genuinely unbuilt.
- Round 2b (record correction, 2026-09-15): **`rmcp` removed from `src-tauri/Cargo.toml` while this page is unbuilt**, as round 1 of this brief instructed. It was declared with `features = ["server", "transport-streamable-http-server"]` and used nowhere, and its comment referred readers to an `mcp/serve.rs` that was never written — a dependency documented by a file that does not exist. To restore it when the listener is built: `rmcp = { version = "3.1.1", features = ["server", "transport-streamable-http-server"] }`, streamable-http rather than stdio because a client cannot spawn this server — the data lives in a turso database the running app already holds. `mcp/decide.rs` is untouched: it is real, tested work and the listener is what is missing, not the decision.
