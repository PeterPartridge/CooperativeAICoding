...
- Round 12 (built): **One source, two honest destinations.** The fetched source no longer has to be a script. One fetch, one digest, one reading-before-use — and then what arrives decides where it can go: a file with a `deny` list is installed into a Solution's `.coperativeai/policy.json`, which both backends already honour on every run, while anything else is a script and takes the WSL route as before. That makes the shareable thing the same thing the runs actually enforce, which is what this brief wanted from the start, and it gives the shared route something the script never had — **it works under Docker**, because a deny list becomes mount masking there where a script could not run at all. Installing is two presses, with the **removals named first**: they are rules somebody believed were being enforced, and one disappearing without a decision is the failure worth a second press. *(Four things worth recording. **The most dangerous wrong answer was nearly the default**: `Policy` defaults its one field, so every JSON object in existence deserializes into an empty policy quite happily — accepting that would let a `package.json` install as a boundary that looks applied and holds nothing, so the check requires a `deny` array to be *present*. **What is installed is re-serialised from the parsed rules, never the file copied**, so what lands is exactly what was previewed and nothing its author put beside `deny` travels into somebody's repository having been reviewed as a deny list. **Rules are compared after tidying**, so `secrets/` and `secrets` are one rule rather than a removal and an addition — a diff full of false movement is one nobody reads. And **the button that cannot act is gone rather than present and failing**: running a deny list as a shell script would do nothing useful and might do something harmful. The script route is kept because it was asked for on purpose. Still not unified, deliberately: the source setting is app-wide while policy files are per-Solution, so a run records which policy file bounded it rather than which source it came from.)*
