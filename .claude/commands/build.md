...
0. **Scaffold (first build in a solution only)** — if the solution's repo has no
   ...
   fields the spec fills in itself as overrides. Use its `commands` to verify the
   skeleton builds and its tests run before calling scaffolding done. If the spec
   has no scaffold block and no boilerplate fits, stop and ask.
1. **Plan** — restate a one-paragraph summary plus bullet-point changes (one per use case).
   ...
   plan, and stop and ask if the location is missing or not checked out. Check
   `template/claude-only/Code_map.md` (if it exists): reuse an existing method
